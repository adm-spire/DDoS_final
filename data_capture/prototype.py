    ...
    try:
        if "IP" in packet and ("TCP" in packet or "UDP" in packet):
            ...
            if last_time:
                ...
                flow_stats[flow_key]["Fwd IATs"].append(iat) if src_ip == packet.ip.src else flow_stats[flow_key]["Bwd IATs"].append(iat)
            ...
            if src_ip == packet.ip.src:
                ...
                if int(packet[protocol].flags, 16) & 0x10:  # Checking if it's a data packet (ACK)
...
for flow_key, stats in flow_stats.items():
    duration = (stats["Last Packet Time"] - stats["Start Time"]) if stats["Last Packet Time"] else 0
    feature_vector = {
        "Flow Duration": duration,
        "Flow Bytes/s": stats["Total Length of Fwd Packets"] / duration if duration > 0 else 0,
        "Flow Packets/s": (len(stats["Packet Lengths"]) / duration) if duration > 0 else 0,
